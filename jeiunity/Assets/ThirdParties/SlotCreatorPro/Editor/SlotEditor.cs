...
public class SlotEditor : Editor 
{
	...
	public override void OnInspectorGUI() { 
		...
		{
			GUI.color = Color.red; 
			//GUI.DrawTexture(new Rect(0,0,0,0), EditorGUIUtility.whiteTexture );
			//GUI.color = Color.white;
		}
		configIssues.Clear();

		Undo.RecordObject(slot, "Slots Creator Change");
...
//		slot.GetComponent<SlotCredits>().maxBetPerLine = EditorGUILayout.IntSlider(new GUIContent("Max Bet Per Line", SlotHelp.MAX_BET_PER_LINE), slot.GetComponent<SlotCredits>().maxBetPerLine,1,20);
		...
		if ((slot.callbackScriptName != null) && (slot.callbackScriptName != ""))
		{
			EditorGUILayout.LabelField("Callback Handler: " + slot.callbackScriptName);
			
			if (GUILayout.Button("Clear", GUILayout.Width(100)))
			{
				slot.callbackScriptName = null;
				callbackScript = null;
			}
		} else {
			callbackScript = (MonoScript)EditorGUILayout.ObjectField(new GUIContent("Callback Handler", SlotHelp.CALLBACKS), callbackScript, typeof(MonoScript), true);
			if (callbackScript != null) slot.callbackScriptName = callbackScript.name;
		}
		EditorGUILayout.EndHorizontal();
	*/
		/*
	...
	void foldoutSymbolSetup()
	{
		...
		for (int i = 0; i < slot.symbolPrefabs.Count; i++)
		{
			...
			if (slot.symbolPrefabs[i] != null)
			{
				...
				if (currentSymbol.linked)
				{
					currentSymbol.linkPosition = (LinkPosition)EditorGUILayout.EnumPopup(currentSymbol.linkPosition, GUILayout.MaxWidth(150));
					if (currentSymbol.linkPosition != LinkPosition.Top)
					{
						EditorGUILayout.LabelField(new GUIContent("Next Link:", SlotHelp.LINK_NEXT), EditorStyles.miniLabel, GUILayout.Width(70));
				...
				//currentSymbol.exclusive = EditorGUILayout.ToggleLeft(new GUIContent("Exclusive"),currentSymbol.exclusive, EditorStyles.miniLabel, GUILayout.Width(75));
	...
	void showSymbolSetFoldout(int index)
	{
		...
		/*
		EditorGUI.indentLevel++;
		slot.setPays[index].paySound = (AudioClip)EditorGUILayout.ObjectField(new GUIContent("Pay Sound", SlotHelp.SET_ANTICIPATION), slot.setPays[index].paySound, typeof(AudioClip), false);
		if (slot.symbolSets[index].typeofSet == SetsType.scatter)
	...
	void checkErrors()
	{
		...
		for (int symbolIndex = 0; symbolIndex < slot.symbolPrefabs.Count; symbolIndex++)
		{
			...
			if (symbolInfo.linked == true) {
				...
				{
					SlotSymbolInfo symbolInfoCompare = slot.symbolInfo[si];

					if (!symbolInfoCompare.linked) continue;
					if (linkName == symbolInfoCompare.linkName)
					{
						if (symbolsInChain.Contains(symbolInfoCompare.link)) dupeSymbol = true; else symbolsInChain.Add (symbolInfoCompare.link);
