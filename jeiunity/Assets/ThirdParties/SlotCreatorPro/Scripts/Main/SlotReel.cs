...
public class SlotReel : MonoBehaviour {
	...
	void OnEnable()
	{
		...
		heightOffset = -transform.parent.transform.position.y + (symbolHeight * (slot.reelHeight / 2) + (symbolPadding * (slot.reelHeight / 2)));
		widthOffset = -transform.parent.transform.position.x + (symbolWidth * (slot.numberOfReels / 2) + (reelPadding * (slot.numberOfReels / 2)));
		Destroy (symb);
	...
	{
		transform.position = new Vector3((-widthOffset + (symbolWidth * (reelIndex - 1)) + (reelPadding * (reelIndex - 1))),transform.position.x, transform.position.z);
	...
	/*
	public int[,] getSymbols(int[,] symbolArray)
	{
		int[,] result = new int[symbolHeight, symbolWidth];
		for(int i = 0; i < symbolArray.Length; i++)
		{
			for (int ii = 0; ii < symbolArray.GetLength(i); ii++)
			{
				int chosen = -1;
				for (int index = 0; index < cumulativeFrequencyList.Count; index++)
				{
					if (symbolArray[i,ii] <= cumulativeFrequencyList[index]) { chosen = index; break; }
				}
				result[i,ii] = chosen;
			}
		}
	...
	*/
	...
	public int getSymbol()
	{
		...
		if ((slot.symbolInfo[lastSelected].linked) && (slot.symbolInfo[lastSelected].linkPosition != LinkPosition.Top))
		{
			...
			lastSelected = chosen;
			//Debug.Log("return linked symbol: " + chosen);
			return chosen;
		}
	...
	void createSymbol(int slotIndex)
	{
		...
		if (slot.useSuppliedResult)
		{
			...
			{
				if (slot.suppliedResult[reelIndex-1,symbolsSpinRemaining-1] > -1)
				{
		...
		if (symb.GetComponent<SpriteRenderer>())
			symb.transform.localPosition = new Vector3(slot.reelCenter.position.x + 0,slot.reelCenter.position.y + (-heightOffset + (slotIndex * symbolHeight - transform.position.y)) + (symbolPadding * slotIndex), 0);
		if (symb.GetComponent<MeshFilter>())
			symb.transform.localPosition = new Vector3(slot.reelCenter.position.x + 0,slot.reelCenter.position.y + (-heightOffset + (slotIndex * symbolHeight - transform.position.y)) + (symbolPadding * slotIndex), 0);
	...
	public void spinReel(float spinTime)
	{
		...
		transform.DOMoveY(-symbolHeight, speed).OnComplete(OnNextSymbol).SetRelative(true).SetEase(Ease.Linear);
	...
	void OnNextSymbol()
	{
		...
		if (stopped)
		{
			transform.DOMoveY(-symbolHeight - symbolPadding, slot.easeOutTime).SetRelative(true).OnComplete(OnReelStopped).SetEase(slot.reelEase);
			...
			if (anticipation)
			{
				transform.DOMoveY(-symbolHeight - symbolPadding, speed/2.0f).SetRelative(true).OnComplete(OnNextSymbol).SetEase(Ease.Linear);
				...
				transform.DOMoveY(-symbolHeight - symbolPadding, speed).SetRelative(true).OnComplete(OnNextSymbol).SetEase(Ease.Linear);
